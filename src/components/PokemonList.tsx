...
function PokemonList({ pokemons }: PokemonListProps) {
  ...
  const defaultPerPage = searchParams.get('perPage') || 12;
    ...
    <>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-2 p-2">
        { pokemonsFiltered.map((pokemon) => (
          <Link
            key={ `${pokemon.pokedex_id}${pokemon.name.fr}` }
            href={ `/pokemon/${pokemon.pokedex_id}-${pokemon.name.fr.toLowerCase()}` }
            className="flex"
          >
